...
export const useAdvancedFacialRecognition = () => {
  ...
  const calculateImageQuality = (
    ...
    if (width >= 640 && height >= 480) quality += 0.2;
    if (width >= 1024 && height >= 768) quality += 0.1;
    
    // Check face size (should be reasonably large)
    if (bbox) {
      const faceArea = (bbox.width * bbox.height) / (width * height);
      if (faceArea > 0.1) quality += 0.2; // Face takes up >10% of image
      if (faceArea > 0.2) quality += 0.1; // Face takes up >20% of image
    }
