...
const FacialRecognition = ({ 
  ...
  const { isProcessing, recognizeFace, capturePhoto: capturePhotoHook, videoRef } = useFacialRecognition();
  ...
  const processImage = async () => {
    ...
    if (mode === 'register') {
      ...
      if (capturedImage) {
        imageData = capturedImage.toDataURL('image/jpeg', 0.8);
