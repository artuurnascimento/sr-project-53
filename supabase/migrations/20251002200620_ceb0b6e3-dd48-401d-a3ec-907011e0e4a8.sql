...
-- Drop existing policies on facial_recognition_audit
...
-- Add index for performance on access log queries
CREATE INDEX IF NOT EXISTS idx_biometric_access_log_accessed_by 
ON public.biometric_access_log(accessed_by, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_biometric_access_log_profile 
